...
## 📊 Аналитика
Все действия пользователя фиксируются и мгновенно попадают в Google Sheets: лист «События» хранит подробный лог, а сводка «Аналитика» автоматически обновляется без ручных выгрузок. Заказчик получает доступ к актуальным цифрам — распределению по полу, доле поделившихся телефоном, возвратам через 24 часа, среднему числу примерок, доле завершивших подбор, CTR кнопки «Записаться» и конверсии в запись. Диаграммы в таблице обновляются по мере активности аудитории.
